# -*- coding: utf-8 -*-
"""
Created on Thu Aug 29 17:31:54 2024
...
for idx, country_name in enumerate(countries):
    fig, (ax0, ax1) = plt.subplots(2, 1, dpi=600, gridspec_kw={'height_ratios': [1, 4]}, sharex=True, figsize=(cm2inch(19.0), cm2inch(10.0)))
    
    l1 = ax1.plot(month_media_coverage[idx]['Month-Year'], month_media_coverage[idx]['Ratio_count/#_newspapers'], color='C0', label='Number of articles / number of newspapers')
    ax2 = ax1.twinx()
    l2 = ax2.plot(month_socmedia_coverage[idx]['Month-Year'], month_socmedia_coverage[idx]['Ratio_count/#_twitterusers'], color='C1', label='Number of tweets / number of Twitter users')
    ...
    ax1.set_xticklabels(['08-2022', '09-2022', '10-2022'])
    ...
    for xmin in ax1.xaxis.get_minorticklocs():
        ax0.axvline(x=xmin, lw=0.5, ls='-', color='lightgray', alpha=0.5, zorder=0)

    ax0.set_yticks([])
    ax0.set_yticklabels([])
    
    # Title with the country name, centered and larger font size
    ax0.set_title(country_name, loc='center', fontsize=16, pad=20)
    ...
    fig.savefig(os.path.join(dir_output, f'Time_analysis_{country_name}.png'), dpi=600, bbox_inches='tight')