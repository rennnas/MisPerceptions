...
for idx, ii in enumerate(time_coverage_media):
    ...
    df_cov['date'] = pd.to_datetime(df_cov['date'])  # Specify the correct format here
...
for ttt in time_coverage_socmedia:
    ...
    tweets_current['Timestamp'] = pd.to_datetime(
        ttt['created_at'])  # Specify the correct format here
    ...
    for index, row in df_soccov.iterrows():
        # aa = row['Count']/numb_twitter_users[numb_twitter_users['Year'].values==row['Year']]['Global']
...
for country_name in countries:
    ...
    for index, row in events_socmedia[ii].iterrows():
    ...
    ax1.legend(handles=handles, loc='upper center', bbox_to_anchor=(0.5, -0.2),
               fancybox=True, ncol=5)  # Adjusted bbox_to_anchor

    ax0.legend(handles=handles_evs, loc='upper center', bbox_to_anchor=(0.5, 1.0),
               fancybox=True, ncol=3)  # Adjusted bbox_to_anchor

    ax0.get_legend().set_title(country_name)

    fig.savefig(os.path.join(dir_output, 'Time_analysis_' +
