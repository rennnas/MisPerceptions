# -*- coding: utf-8 -*-
"""
Created on Sun Sep  1 12:38:23 2024
...
ii = 0
for country_name in countries:
    # Increase the figure size to accommodate both the main plot and the zoom-in plot
    fig, ax1 = plt.subplots(dpi=600, figsize=(cm2inch(22.0), cm2inch(10.0)))  # Increased width
    ...
    highlight_start = pd.to_datetime('2022-08-01').to_pydatetime()
    highlight_end = pd.to_datetime('2022-10-31').to_pydatetime()
    ...
    highlight_patch = Line2D([0], [0], color='red', alpha=0.2, label='Aug - Oct, 2022', linestyle='--')
    ...
    # Create a zoom-in subplot outside the main plot area
    inset_ax = fig.add_axes([1.0, 0.8, 0.3, 0.3])  # Adjust [left, bottom, width, height] for larger size and right positioning

    # Plot both lines in the zoomed-in view
    inset_ax.plot(month_media_coverage[ii]['Month-Year'], month_media_coverage[ii]['Ratio_count/#_newspapers'], color='C0')
    inset_ax.plot(month_socmedia_coverage[ii]['Month-Year'], month_socmedia_coverage[ii]['Ratio_count/#_twitterusers'], color='C1')

    # Set limits to zoom into the highlighted area
    inset_ax.set_xlim([highlight_start, highlight_end])
    inset_ax.set_ylim([0, max(month_media_coverage[ii]['Ratio_count/#_newspapers'].max(), month_socmedia_coverage[ii]['Ratio_count/#_twitterusers'].max())])

    # Add gridlines and format them
    inset_ax.grid(color='lightgray', linewidth=1)

    # Add vertical grid lines for months in the zoomed-in view
    for month in pd.date_range(start=highlight_start, end=highlight_end, freq='MS'):
        inset_ax.axvline(x=month.to_pydatetime(), color='lightgray', linestyle='--', alpha=0.5, linewidth=0.5)

    # Add title and labels to the inset plot
    inset_ax.set_title('Selected period', fontsize=8)
    inset_ax.set_xlabel('Date', fontsize=6)
    inset_ax.set_ylabel('Ratio', fontsize=6)
    inset_ax.tick_params(axis='x', labelsize=2)
