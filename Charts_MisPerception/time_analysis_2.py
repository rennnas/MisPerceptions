...
for country_name in countries:
    ...
    # Align horizontal grid lines for both y-axes
    ...
    for index, row in events_socmedia[ii].iterrows():
        ...
        ax0.axvline(x=row['# month'], lw=1, ls='-', color=color_list[cc], alpha=0.5)
        ax1.axvline(x=row['# month'], lw=1, ls='-', color=color_list[cc], alpha=0.5)
