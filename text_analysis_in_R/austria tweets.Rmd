...
```{r}
austria_tweets <- read.csv("austria_geolocated_2010_2022_anonymous.csv")
```



```{r}
text_corpus_at_tweets <- corpus(austria_tweets, text_field = "text")
text_dfm_at <- dfm(text_corpus_at_tweets, stem = TRUE, remove = stopwords("de"), remove_punct = TRUE)
...
```{r}
dict_topic_dtm_at_tweets <- dfm_lookup(text_dfm_at, dict_topic_de, exclusive = TRUE)

```

```{r}
df_at_tw <- convert(dict_topic_dtm_at_tweets, to = "data.frame")
```
...
df_at_tweets <- df3 %>%
  ...
  summarize(across(a:q, sum, na.rm = TRUE)) %>%
  ungroup()
